...
BiNode *deleteNode(BiTree root, int val) {
  if (!root) {
    return NULL;
  }
  ...
  if (val > root->data) {
    ...
    if (!root->lchild) {
      BiNode *rightchild = root->rchild;
      delete root;
      return rightchild;
    }
    if (!root->rchild) {
      BiNode *leftchild = root->lchild;
      delete root;
      return leftchild;
    }

    BiNode *lmaxNode = root->lchild;
    while (lmaxNode->rchild) {
      lmaxNode = lmaxNode->rchild;
    }

    root->data = lmaxNode->data;
    root->lchild = deleteNode(root->lchild, lmaxNode->data);
  }
  return root;
}
